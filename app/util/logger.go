...
func ThrowError(ed ErrorInfo, msg string) error {
	return fmt.Errorf("%s.%s  -  failed, because: %s", ed.FileName, ed.FunctionName, msg)
}

// ForwardError update thrown error with call stack parent
func ForwardError(ed ErrorInfo, err error) error {
	return fmt.Errorf("%s.%s  -  failed, because: \n\t%s", ed.FileName, ed.FunctionName, err)
}
